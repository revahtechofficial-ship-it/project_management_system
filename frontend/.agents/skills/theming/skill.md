# Flutter Theming & Visual Design Skill

## Description
Active reference for visual design, `ThemeData`/Material 3 theming, component
styling, color schemes, and typography. Load this skill whenever building or
adjusting the look and feel of the app: themes, colors, fonts, styling code, or
general UI polish.
...
* **Dark/Light Mode and Theme Toggle:** Implement support for both light and
  ...
  `themeMode` property can be dynamically controlled (e.g., via a `riverpod`
  provider) to allow for toggling between `ThemeMode.light`, `ThemeMode.dark`,
  or `ThemeMode.system`.
