...
## Description
Active reference for setting up navigation. Load this skill when wiring app
navigation: declarative routes, deep links, route parameters, authentication
redirects, or short-lived `Navigator` screens. Core rule (also in AGENTS.md §9):
use `go_router` for app navigation and the built-in `Navigator` for ephemeral
screens.
