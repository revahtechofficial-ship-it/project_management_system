  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      ...
      child: Padding(
        ...
        // On a phone there is no room for the label and the date beside the
        // time, so a tight bar keeps only what a status strip must have: the
        // time and the NPT badge. Given room, the label and the Nepal date
        // rejoin it. Everything that can grow is Flexible, so the bar can never
        // overflow — it sheds text, it does not spill.
        child: LayoutBuilder(
          builder: (BuildContext context, BoxConstraints c) {
            final bool tight = c.maxWidth < 440;
            return Row(
              children: <Widget>[
                Container(
                  width: 34,
